...
class HTML_joomla_flash_uploader {
	function showFlash($row, $realfolder, $use_js_include, $jfu_config, $plugin=false, $is_editor=false) {		
    ...
    $Cfg_lang = $language->getTag();
        ...
        if ($row->allowed_file_extensions == '') {
          ...
          $extra_settings_js .= 'flashvars.hide_upload="true";';
        }
        if ($row->show_size == 'false') {
          $extra_settings .= '&amp;show_size=false';
          $extra_settings_js .= 'flashvars.show_size="false";';
        }
        if ($row->show_server_date_instead_size == 'true') {
          $extra_settings .= '&amp;show_server_date_instead_size=true';
          $extra_settings_js .= 'flashvars.show_server_date_instead_size="true";';
        }
		...
		// now I overwrite the existing function to trigger it by the flash
    echo '
      ...
      if (acivateKeep && (oldKeepAlive != null)) { 
		...
		$objecttag = '
          <object name="mymovie" classid="clsid:d27cdb6e-ae6d-11cf-96b8-444553540000" codebase="http://fpdownload.macromedia.com/pub/shockwave/cabs/flash/swflash.cab#version=8,0,0,0" width="'.$width.'" height="'.$height.'"  align="middle">
            ...
            <param name="movie" value="'.$relative_dir.$prefix_path.'components/com_jfuploader/tfu/tfu_3.2.swf?joomla=frontend&amp;session_id='.$sess_id.'&amp;lang='.$lang.'&amp;base='.$base_dir.'&amp;relative_dir='.$relative_dir.$extra_settings.'&amp;'.$row->swf_text.'" />
            <param name="quality" value="high" /><param name="bgcolor" value="#ffffff" />';
             $wmode = '';
             if ($row->fix_overlay == "true" || $is_editor) {
            ...
            <embed src="'.$relative_dir.$prefix_path.'components/com_jfuploader/tfu/tfu_3.2.swf?joomla=frontend&amp;session_id='.$sess_id.'&amp;lang='.$lang.'&amp;base='.$base_dir.'&amp;relative_dir='.$relative_dir.$extra_settings.'&amp;'.$row->swf_text.'" name="mymovie" quality="high" bgcolor="#ffffff" width="'.$width.'" height="'.$height.'" align="middle" '. (($width == '650') ? 'scale="noScale" ' : '') . ' allowfullscreen="true" allowScriptAccess="sameDomain" ' . $wmode . ' type="application/x-shockwave-flash" pluginspage="http://www.macromedia.com/go/getflashplayer" />
            </object> ';
		...
		if ($use_js_include) { // include with Javascript
			...
			<script type="text/javascript">
			   document.write(\'<div style="height:'.$height.'px;" id="flashcontent"><div class="noflash">' . str_replace( "'", '"', JText::_('MES_NO_FLASH')) . '<\/div><\/div>\');
                var flashvars = {};
                var params = {};    
                ...
                $output .= '  swfobject.embedSWF("'.$relative_dir.$prefix_path.'components/com_jfuploader/tfu/tfu_3.2.swf", "flashcontent", "'.$width.'", "'.$height.'", "8.0.0", "", flashvars, params, attributes);
                ';		   
                $output .= '
			</script>
...
$tfu_enable_resize_att = ($tfu_enable_resize == 0) ? ' disabled="disabled" ' : '';
    echo '<style>body { padding-left: 0px; padding-right: 0px; }</style>';
    echo '<script type="text/Javascript">
         ...
         if ($tfu_show_caption) {
             echo '<input onkeyup="enableCaption();" class="jfu_input" style="width:225px;" type="text" name="jfu_alt" id="jfu_alt"></input> <input type="checkbox" name="jfu_as_caption" disabled="disabled" onclick="checkCaption();" id="jfu_as_caption"><label class="nowrap" for="jfu_as_caption"><img id="jfu_caption" alt="'.JText::_('JFU_XTD_CAPTION').'" title="'.JText::_('JFU_XTD_CAPTION').'"  class="jfu_caption" src="components/com_jfuploader/images/1x1.gif"></img></label>';
         } else {
         ...
         if ($tfu_show_alignment == 1 || $tfu_show_spacing == 1) {
             ...
             echo  JText::_('JFU_XTD_HSPACE').'<br/><input maxlength="8" class="jfu_fade jfu_input" onfocus="if (this.value == \''.JText::_('JFU_XTD_SPACE_DEFAULT').'\') {this.value = \'\'; this.className = \'jfu_not_fade jfu_input\';}" style="width:103px;" type="text" name="jfu_hspace" id="jfu_hspace" value="'.JText::_('JFU_XTD_SPACE_DEFAULT').'"></input>
                   <br/>'.JText::_('JFU_XTD_VSPACE').'<br/><input maxlength="8" class="jfu_fade jfu_input" onfocus="if (this.value == \''.JText::_('JFU_XTD_SPACE_DEFAULT').'\') {this.value = \'\'; this.className = \'jfu_not_fade jfu_input\';}" style="width:103px;" type="text" name="jfu_vspace" id="jfu_vspace" value="'.JText::_('JFU_XTD_SPACE_DEFAULT').'"></input>';
             }
         ...
         echo '<div class="jfu_image_buttons_right">       
           <input type="button"  class="btn" style="width:120px;" id="jfu_insert_image"  disabled="disabled" onclick="insert();" value="'.JText::_('JFU_XTD_INSERT_IMAGE').'"></input>';  
         if ($tfu_show_thumbnail_create) {
           echo '<br/><input  class="btn" type="button" style="width:120px;margin-top:10px;" id="jfu_create_thumb" disabled="disabled" onclick="createThumbnail();" value="'.JText::_('JFU_XTD_CREATE_THUMB').'"></input>';
         }
         echo '<br/><input  class="btn" type="button" style="width:120px;margin-top:10px;" onclick="gotoPage(\'link\');" value="'.JText::_('JFU_XTD_SHOW_LINK_SETTINGS').'"></input>';
         if ($tfu_show_help == 1) {
           echo '<br/><input  class="btn" type="button" style="width:120px;margin-top:10px;" id="jfu_help" onclick="window.open(\'http://www.tinywebgallery.com/en/tfu/web_jfu.php#jfu_xtd\')" value="'.JText::_('JFU_XTD_HELP').'"></input>';
         }
         echo '
           </div> 
         </div>
  ...
  echo '<div id="jfu_image_div_panel" class="jfu_image_div_panel_bottom">
         ...
         <input class="middle" disabled="disabled" type="checkbox" name="jfu_google_doc" id="jfu_google_doc"> <label class="middle nowrap" for="jfu_google_doc">'.JText::_('JFU_XTD_GOOGLE_TEXT').'</label><img  id="jfu_info" alt="'.JText::_('JFU_XTD_GOOGLE_INFO').'" title="'.JText::_('JFU_XTD_GOOGLE_INFO').'"  class="jfu_info" src="components/com_jfuploader/images/1x1.gif"></img>
         </div>
         </div>'; 
         
         echo '<div class="jfu_image_buttons_right">
         <input type="button"  class="btn" style="width:120px;" id="jfu_insert_link" onclick="insert();" disabled="disabled" value="'.JText::_('JFU_XTD_INSERT_IMAGE').'"></input>
         <br/><input type="button"  class="btn" style="width:120px;margin-top:10px;" id="jfu_insert_link_button" onclick="insertLink();" disabled="disabled" value="'.JText::_('JFU_XTD_INSERT_LINK_BUTTON').'"></input>
         <br/><input type="button"  class="btn" style="width:120px;margin-top:10px;" onclick="gotoPage(\'image\');" value="'.JText::_('JFU_XTD_SHOW_IMAGE_SETTINGS').'"></input>';
         if ($tfu_show_help == 1) {
         echo '
            <br/><input  class="btn" type="button" style="width:120px;margin-top:10px;" id="jfu_help" onclick="window.open(\'http://www.tinywebgallery.com/en/tfu/web_jfu.php#jfu_xtd\')" value="'.JText::_('JFU_XTD_HELP').'"></input>';
         }
             echo '</div>';
         echo '</div>';
