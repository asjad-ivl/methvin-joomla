...
function modChrome_round_box($module, &$params, &$attribs) {
	...
	if(strrpos($template_description,"callie") <= 0 && strrpos($template_description,"Callie") <= 0 && strrpos($template_description,"compassion") <= 0 && strrpos($template_description,"Compassion") <= 0 && $template_creation_date != "November 2011" && strrpos($template_description,"presti") <= 0 && strrpos($template_description,"Presti") <= 0 && $template_creation_date != "February 2013" && strrpos($template_description,"maxed") <= 0 && strrpos($template_description,"Maxed") <= 0 && $template_creation_date != "September 2011") {
		...
		if(strrpos($suffix,"5_icon.") >= 1) {
			$suffix_expand_ion = explode(" ", $suffix);
			if(strrpos($suffix_expand_ion[0],"5_icon.") >= 1) {
				$suffix_ion_class = $suffix_expand_ion[0];
				$suffix_new_ion = "";
				if ($suffix_expand_ion[1] != "") {
					$suffix_new_ion = $suffix_expand_ion[1];
				}
				if ($suffix_expand_ion[2] != "") {
					$suffix_new_ion = $suffix_new_ion.' '.$suffix_expand_ion[2];
				}
				if ($suffix_expand_ion[3] != "") {
					$suffix_new_ion = $suffix_new_ion.' '.$suffix_expand_ion[3];
				}
				if ($suffix_expand_ion[4] != "") {
					$suffix_new_ion = $suffix_new_ion.' '.$suffix_expand_ion[4];
				}
				$suffix = $suffix_new_ion;
			}
			else if(strrpos($suffix_expand_ion[1],"5_icon.") >= 1) {
				$suffix_ion_class = $suffix_expand_ion[1];
				$suffix_new_ion = "";
				if ($suffix_expand_ion[0] != "") {
					$suffix_new_ion = $suffix_expand_ion[0];
				}
				if ($suffix_expand_ion[2] != "") {
					$suffix_new_ion = $suffix_new_ion.' '.$suffix_expand_ion[2];
				}
			...
			else if(strrpos($suffix_expand_ion[2],"5_icon.") >= 1) {
				$suffix_ion_class = $suffix_expand_ion[2];
				$suffix_new_ion = "";
				if ($suffix_expand_ion[0] != "") {
					$suffix_new_ion = $suffix_expand_ion[0];
				}
				if ($suffix_expand_ion[1] != "") {
					$suffix_new_ion = $suffix_new_ion.' '.$suffix_expand_ion[1];
				}
				if ($suffix_expand_ion[3] != "") {
					$suffix_new_ion = $suffix_new_ion.' '.$suffix_expand_ion[3];
				}
			...
			else if(strrpos($suffix_expand_ion[3],"5_icon.") >= 1) {
				$suffix_ion_class = $suffix_expand_ion[3];
				$suffix_new_ion = "";
				if ($suffix_expand_ion[0] != "") {
					$suffix_new_ion = $suffix_expand_ion[0];
				}
			...
			else if(strrpos($suffix_expand_ion[4],"5_icon.") >= 1) {
				$suffix_ion_class = $suffix_expand_ion[4];
				$suffix_new_ion = "";
				if ($suffix_expand_ion[0] != "") {
					$suffix_new_ion = $suffix_expand_ion[0];
				}
		...
		if(strrpos($suffix_ion_class,"5_icon.") >= 1) {
			$suffix_ion_class = str_replace("s5_icon.","",$suffix_ion_class);
		}

		if(strrpos($suffix," ") > 1 || ctype_xdigit($suffix)) {
			$suffix_expand = explode(" ", $suffix);
			$suffix_expand0 = $suffix_expand[0];
			$suffix_expand1 = $suffix_expand[1];
			$suffix_expand2 = $suffix_expand[2];
			$suffix_expand3 = $suffix_expand[3];
			$suffix_expand4 = $suffix_expand[4];
			$suffix_expand5 = $suffix_expand[5];
			if(strlen($suffix_expand0) == 6 && $suffix_expand0[0] != "-" && ctype_xdigit($suffix_expand0)) {
				$suffix = $suffix_expand[1];
				if ($suffix_expand[2]) {
					$suffix = $suffix.' '.$suffix_expand[2];
				}
				if ($suffix_expand[3]) {
					$suffix = $suffix.' '.$suffix_expand[3];
				}
				if ($suffix_expand[4]) {
					$suffix = $suffix.' '.$suffix_expand[4];
				}
				$suffix_color = $suffix_expand[0];
			}
			else if(strlen($suffix_expand1) == 6 && $suffix_expand1[0] != "-" && ctype_xdigit($suffix_expand1)) {
				$suffix = $suffix_expand[0];
				if ($suffix_expand[2]) {
					$suffix = $suffix.' '.$suffix_expand[2];
				}
				if ($suffix_expand[3]) {
				...
				$suffix_color = $suffix_expand[1];
			}
			else if(strlen($suffix_expand2) == 6 && $suffix_expand2[0] != "-" && ctype_xdigit($suffix_expand2)) {
				$suffix = $suffix_expand[0];
				if ($suffix_expand[1]) {
					$suffix = $suffix.' '.$suffix_expand[1];
				}
				if ($suffix_expand[3]) {
					$suffix = $suffix.' '.$suffix_expand[3];
				}
				...
				$suffix_color = $suffix_expand[2];
			}
			else if(strlen($suffix_expand3) == 6 && $suffix_expand3[0] != "-" && ctype_xdigit($suffix_expand3)) {
				$suffix = $suffix_expand[0];
				if ($suffix_expand[1]) {
					$suffix = $suffix.' '.$suffix_expand[1];
				}
				...
				$suffix_color = $suffix_expand[3];
			}
			else if(strlen($suffix_expand4) == 6 && $suffix_expand4[0] != "-" && ctype_xdigit($suffix_expand4)) {
				$suffix = $suffix_expand[0];
				if ($suffix_expand[1]) {
					$suffix = $suffix.' '.$suffix_expand[1];
				}
				...
				<div class="s5_module_box_1">
					<div class="s5_module_box_2">
						...
						<div <?php if ($suffix_color != "no") { ?>style="background:#<?php echo $suffix_color; ?>" <?php } ?>class="s5_mod_h3_outer">
							...
							<i class="s5_mod_ion_icon <?php echo $suffix_ion_class; ?>"></i>
							<?php } ?>
