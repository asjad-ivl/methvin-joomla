...
?>
    <?php foreach ($this->items as $i => $item): ?>
        <tr class="row<?php echo $i % 2; ?>">
            ...
            <td class="left">
                <input type="hidden" name="input_prefixes[]" id="input_prefixes" value="<?php echo $item->prefix; ?>"  >
            ...
            <td class="left">
                <div class="btn-group">            
                    <a class="btn btn-mini btn-primary modal" title="<?php echo JText::_('COM_SPTRANSFER_CHOOSE'); ?>" href="<?php echo JRoute::_('index.php?option=com_sptransfer&amp;view=table&amp;layout=default&amp;tmpl=component&amp;prefix=' . $item->prefix . '&amp;name=' . $item->name); ?>" onclick="return false;" rel="{handler: 'iframe', size: {x: 900, y: 400}}"><?php echo JText::_('COM_SPTRANSFER_CHOOSE'); ?></a>
                    <a class="btn btn-mini" title="<?php echo JText::_('JCLEAR'); ?>" href="#" onclick="jClearItem('<?php echo $item->prefix; ?>', '<?php echo $item->name; ?>');"><?php echo JText::_('JCLEAR'); ?></a></div>
                </div>
            </td>
        </tr>
    <?php endforeach; ?>
