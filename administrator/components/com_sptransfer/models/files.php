...
class SPTransferClientFtp {
    ...
    public function connect($host = '127.0.0.1', $port = 21) {
        ...
        if (FTP_NATIVE) {
            ...
            if (($this->_conn === false) && (function_exists('ftp_ssl_connect')))
                $this->_conn = @ftp_ssl_connect($host, $port, $this->_timeout);

            if ($this->_conn === false) {
                JLog::add(JText::sprintf('JLIB_CLIENT_ERROR_JFTP_NO_CONNECT', $host, $port), JLog::WARNING, 'com_sptransfer');
        ...
        if (!$this->_conn) {
            JLog::add(JText::sprintf('JLIB_CLIENT_ERROR_JFTP_NO_CONNECT_SOCKET', $host, $port, $errno, $err), JLog::WARNING, 'com_sptransfer');
        ...
        if (!$this->_verifyResponse(220)) {
            JLog::add(JText::sprintf('JLIB_CLIENT_ERROR_JFTP_BAD_RESPONSE', $this->_response), JLog::WARNING, 'com_sptransfer');
    ...
    public function login($user = 'anonymous', $pass = 'jftp@joomla.org') {
        ...
        if (FTP_NATIVE) {
            if (@ftp_login($this->_conn, $user, $pass) === false) {
                JLog::add('JFTP::login: Unable to login', JLog::WARNING, 'com_sptransfer');
        ...
        if (!$this->_putCmd('USER ' . $user, array(331, 503))) {
            JLog::add(JText::sprintf('JLIB_CLIENT_ERROR_JFTP_BAD_USERNAME', $this->_response, $user), JLog::WARNING, 'com_sptransfer');
        ...
        if (!$this->_putCmd('PASS ' . $pass, 230)) {
            JLog::add(JText::sprintf('JLIB_CLIENT_ERROR_JFTP_BAD_PASSWORD', $this->_response, str_repeat('*', strlen($pass))), JLog::WARNING, 'com_sptransfer');
    ...
    public function pwd() {
        ...
        if (FTP_NATIVE) {
            if (($ret = @ftp_pwd($this->_conn)) === false) {
                JLog::add(JText::_('JLIB_CLIENT_ERROR_JFTP_PWD_BAD_RESPONSE_NATIVE'), JLog::WARNING, 'com_sptransfer');
        ...
        if (!$this->_putCmd('PWD', 257)) {
            JLog::add(JText::sprintf('JLIB_CLIENT_ERROR_JFTP_PWD_BAD_RESPONSE', $this->_response), JLog::WARNING, 'com_sptransfer');
    ...
    public function syst() {
        ...
        if (FTP_NATIVE) {
            if (($ret = @ftp_systype($this->_conn)) === false) {
                JLog::add(JText::_('JLIB_CLIENT_ERROR_JFTP_SYS_BAD_RESPONSE_NATIVE'), JLog::WARNING, 'com_sptransfer');
            ...
            if (!$this->_putCmd('SYST', 215)) {
                JLog::add(JText::sprintf('JLIB_CLIENT_ERROR_JFTP_SYST_BAD_RESPONSE', $this->_response), JLog::WARNING, 'com_sptransfer');
    ...
    public function chdir($path) {
        ...
        if (FTP_NATIVE) {
            if (@ftp_chdir($this->_conn, $path) === false) {
                JLog::add(JText::_('JLIB_CLIENT_ERROR_JFTP_CHDIR_BAD_RESPONSE_NATIVE'), JLog::WARNING, 'com_sptransfer');
        ...
        if (!$this->_putCmd('CWD ' . $path, 250)) {
            JLog::add(JText::sprintf('JLIB_CLIENT_ERROR_JFTP_CHDIR_BAD_RESPONSE', $this->_response, $path), JLog::WARNING, 'com_sptransfer');
    ...
    public function reinit() {
        ...
        if (FTP_NATIVE) {
            if (@ftp_site($this->_conn, 'REIN') === false) {
                JLog::add(JText::_('JLIB_CLIENT_ERROR_JFTP_REINIT_BAD_RESPONSE_NATIVE'), JLog::WARNING, 'com_sptransfer');
        ...
        if (!$this->_putCmd('REIN', 220)) {
            JLog::add(JText::sprintf('JLIB_CLIENT_ERROR_JFTP_REINIT_BAD_RESPONSE', $this->_response), JLog::WARNING, 'com_sptransfer');
    ...
    public function rename($from, $to) {
        ...
        if (FTP_NATIVE) {
            if (@ftp_rename($this->_conn, $from, $to) === false) {
                JLog::add(JText::_('JLIB_CLIENT_ERROR_JFTP_RENAME_BAD_RESPONSE_NATIVE'), JLog::WARNING, 'com_sptransfer');
        ...
        if (!$this->_putCmd('RNFR ' . $from, 350)) {
            JLog::add(JText::sprintf('JLIB_CLIENT_ERROR_JFTP_RENAME_BAD_RESPONSE_FROM', $this->_response, $from), JLog::WARNING, 'com_sptransfer');
        ...
        if (!$this->_putCmd('RNTO ' . $to, 250)) {
            JLog::add(JText::sprintf('JLIB_CLIENT_ERROR_JFTP_RENAME_BAD_RESPONSE_TO', $this->_response, $to), JLog::WARNING, 'com_sptransfer');
    ...
    public function chmod($path, $mode) {
        ...
        if (FTP_NATIVE) {
            if (@ftp_site($this->_conn, 'CHMOD ' . $mode . ' ' . $path) === false) {
                if (!IS_WIN) {
                    JLog::add(JText::_('JLIB_CLIENT_ERROR_JFTP_CHMOD_BAD_RESPONSE_NATIVE'), JLog::WARNING, 'com_sptransfer');
                }
                return false;
            }
            return true;
        ...
        if (!$this->_putCmd('SITE CHMOD ' . $mode . ' ' . $path, array(200, 250))) {
            if (!IS_WIN) {
                JLog::add(JText::sprintf('JLIB_CLIENT_ERROR_JFTP_CHMOD_BAD_RESPONSE', $this->_response, $path, $mode), JLog::WARNING, 'com_sptransfer');
            }
            return false;
        }
        return true;
    ...
    public function delete($path) {
        ...
        if (FTP_NATIVE) {
            if (@ftp_delete($this->_conn, $path) === false) {
                if (@ftp_rmdir($this->_conn, $path) === false) {
                    JLog::add(JText::_('JLIB_CLIENT_ERROR_JFTP_DELETE_BAD_RESPONSE_NATIVE'), JLog::WARNING, 'com_sptransfer');
        ...
        if (!$this->_putCmd('DELE ' . $path, 250)) {
            if (!$this->_putCmd('RMD ' . $path, 250)) {
                JLog::add(JText::sprintf('JLIB_CLIENT_ERROR_JFTP_DELETE_BAD_RESPONSE', $this->_response, $path), JLog::WARNING, 'com_sptransfer');
    ...
    public function mkdir($path) {
        ...
        if (FTP_NATIVE) {
            if (@ftp_mkdir($this->_conn, $path) === false) {
                JLog::add(JText::_('JLIB_CLIENT_ERROR_JFTP_MKDIR_BAD_RESPONSE_NATIVE'), JLog::WARNING, 'com_sptransfer');
        ...
        if (!$this->_putCmd('MKD ' . $path, 257)) {
            JLog::add(JText::sprintf('JLIB_CLIENT_ERROR_JFTP_MKDIR_BAD_RESPONSE', $this->_response, $path), JLog::WARNING, 'com_sptransfer');
    ...
    public function restart($point) {
        ...
        if (FTP_NATIVE) {
            if (@ftp_site($this->_conn, 'REST ' . $point) === false) {
                JLog::add(JText::_('JLIB_CLIENT_ERROR_JFTP_RESTART_BAD_RESPONSE_NATIVE'), JLog::WARNING, 'com_sptransfer');
        ...
        if (!$this->_putCmd('REST ' . $point, 350)) {
            JLog::add(JText::sprintf('JLIB_CLIENT_ERROR_JFTP_RESTART_BAD_RESPONSE', $this->_response, $point), JLog::WARNING, 'com_sptransfer');
    ...
    public function create($path) {
        ...
        if (FTP_NATIVE) {
            ...
            if (@ftp_pasv($this->_conn, true) === false) {
                JLog::add(JText::_('JLIB_CLIENT_ERROR_JFTP_CREATE_BAD_RESPONSE_PASSIVE'), JLog::WARNING, 'com_sptransfer');
            ...
            if (@ftp_fput($this->_conn, $path, $buffer, FTP_ASCII) === false) {
                JLog::add(JText::_('JLIB_CLIENT_ERROR_JFTP_CREATE_BAD_RESPONSE_BUFFER'), JLog::WARNING, 'com_sptransfer');
                fclose($buffer);
        ...
        if (!$this->_putCmd('STOR ' . $path, array(150, 125))) {
            ...
            JLog::add(JText::sprintf('JLIB_CLIENT_ERROR_JFTP_CREATE_BAD_RESPONSE', $this->_response, $path), JLog::WARNING, 'com_sptransfer');
        ...
        if (!$this->_verifyResponse(226)) {
            JLog::add(JText::sprintf('JLIB_CLIENT_ERROR_JFTP_CREATE_BAD_RESPONSE_TRANSFER', $this->_response, $path), JLog::WARNING, 'com_sptransfer');
    ...
    public function read($remote, &$buffer) {
        ...
        if (FTP_NATIVE) {
            // Turn passive mode on
            if (@ftp_pasv($this->_conn, true) === false) {
                JLog::add(JText::_('JLIB_CLIENT_ERROR_JFTP_READ_BAD_RESPONSE_PASSIVE'), JLog::WARNING, 'com_sptransfer');
            ...
            if (@ftp_fget($this->_conn, $tmp, $remote, $mode) === false) {
                fclose($tmp);
                JLog::add(JText::_('JLIB_CLIENT_ERROR_JFTP_READ_BAD_RESPONSE_BUFFER'), JLog::WARNING, 'com_sptransfer');
        ...
        if (!$this->_putCmd('RETR ' . $remote, array(150, 125))) {
            @ fclose($this->_dataconn);
            JLog::add(JText::sprintf('JLIB_CLIENT_ERROR_JFTP_READ_BAD_RESPONSE', $this->_response, $remote), JLog::WARNING, 'com_sptransfer');
        ...
        if (!$this->_verifyResponse(226)) {
            JLog::add(JText::sprintf('JLIB_CLIENT_ERROR_JFTP_READ_BAD_RESPONSE_TRANSFER', $this->_response, $remote), JLog::WARNING, 'com_sptransfer');
    ...
    public function get($local, $remote) {
        ...
        if (FTP_NATIVE) {
            // Turn passive mode on
            if (@ftp_pasv($this->_conn, true) === false) {
                JLog::add(JText::_('JLIB_CLIENT_ERROR_JFTP_GET_PASSIVE'), JLog::WARNING, 'com_sptransfer');

                return false;
            }

            if (@ftp_get($this->_conn, $local, $remote, $mode) === false) {
                JLog::add(JText::_('JLIB_CLIENT_ERROR_JFTP_GET_BAD_RESPONSE'), JLog::WARNING, 'com_sptransfer');
        ...
        if (!$fp) {
            JLog::add(JText::sprintf('JLIB_CLIENT_ERROR_JFTP_GET_WRITING_LOCAL', $local), JLog::WARNING, 'com_sptransfer');
        ...
        if (!$this->_putCmd('RETR ' . $remote, array(150, 125))) {
            @ fclose($this->_dataconn);
            JLog::add(JText::sprintf('JLIB_CLIENT_ERROR_JFTP_GET_BAD_RESPONSE_RETR', $this->_response, $remote), JLog::WARNING, 'com_sptransfer');
        ...
        if (!$this->_verifyResponse(226)) {
            JLog::add(JText::sprintf('JLIB_CLIENT_ERROR_JFTP_GET_BAD_RESPONSE_TRANSFER', $this->_response, $remote), JLog::WARNING, 'com_sptransfer');
    ...
    public function store($local, $remote = null) {
        ...
        if (FTP_NATIVE) {
            // Turn passive mode on
            if (@ftp_pasv($this->_conn, true) === false) {
                JLog::add(JText::_('JLIB_CLIENT_ERROR_JFTP_STORE_PASSIVE'), JLog::WARNING, 'com_sptransfer');

                return false;
            }

            if (@ftp_put($this->_conn, $remote, $local, $mode) === false) {
                JLog::add(JText::_('JLIB_CLIENT_ERROR_JFTP_STORE_BAD_RESPONSE'), JLog::WARNING, 'com_sptransfer');
        ...
        if (@ file_exists($local)) {
            ...
            if (!$fp) {
                JLog::add(JText::sprintf('JLIB_CLIENT_ERROR_JFTP_STORE_READING_LOCAL', $local), JLog::WARNING, 'com_sptransfer');

                return false;
            }
        } else {
            JLog::add(JText::sprintf('JLIB_CLIENT_ERROR_JFTP_STORE_FIND_LOCAL', $local), JLog::WARNING, 'com_sptransfer');
        ...
        if (!$this->_putCmd('STOR ' . $remote, array(150, 125))) {
            @ fclose($fp);
            @ fclose($this->_dataconn);
            JLog::add(JText::sprintf('JLIB_CLIENT_ERROR_JFTP_STORE_BAD_RESPONSE_STOR', $this->_response, $remote), JLog::WARNING, 'com_sptransfer');
        ...
        while (!feof($fp)) {
            ...
            do {
                if (($result = @ fwrite($this->_dataconn, $line)) === false) {
                    JLog::add(JText::_('JLIB_CLIENT_ERROR_JFTP_STORE_DATA_PORT'), JLog::WARNING, 'com_sptransfer');
        ...
        if (!$this->_verifyResponse(226)) {
            JLog::add(JText::sprintf('JLIB_CLIENT_ERROR_JFTP_STORE_BAD_RESPONSE_TRANSFER', $this->_response, $remote), JLog::WARNING, 'com_sptransfer');
    ...
    public function write($remote, $buffer) {
        ...
        if (FTP_NATIVE) {
            // Turn passive mode on
            if (@ftp_pasv($this->_conn, true) === false) {
                JLog::add(JText::_('JLIB_CLIENT_ERROR_JFTP_WRITE_PASSIVE'), JLog::WARNING, 'com_sptransfer');
            ...
            if (@ftp_fput($this->_conn, $remote, $tmp, $mode) === false) {
                fclose($tmp);
                JLog::add(JText::_('JLIB_CLIENT_ERROR_JFTP_WRITE_BAD_RESPONSE'), JLog::WARNING, 'com_sptransfer');
        ...
        // Send store command to the FTP server
        if (!$this->_putCmd('STOR ' . $remote, array(150, 125))) {
            JLog::add(JText::sprintf('JLIB_CLIENT_ERROR_JFTP_WRITE_BAD_RESPONSE_STOR', $this->_response, $remote), JLog::WARNING, 'com_sptransfer');
            @ fclose($this->_dataconn);
        ...
        do {
            if (($result = @ fwrite($this->_dataconn, $buffer)) === false) {
                JLog::add(JText::_('JLIB_CLIENT_ERROR_JFTP_WRITE_DATA_PORT'), JLog::WARNING, 'com_sptransfer');
        ...
        if (!$this->_verifyResponse(226)) {
            JLog::add(JText::sprintf('JLIB_CLIENT_ERROR_JFTP_WRITE_BAD_RESPONSE_TRANSFER', $this->_response, $remote), JLog::WARNING, 'com_sptransfer');
    ...
    public function listNames($path = null) {
        ...
        if (FTP_NATIVE) {
            // Turn passive mode on
            if (@ftp_pasv($this->_conn, true) === false) {
                JLog::add(JText::_('JLIB_CLIENT_ERROR_JFTP_LISTNAMES_PASSIVE'), JLog::WARNING, 'com_sptransfer');
            ...
            if (($list = @ftp_nlist($this->_conn, $path)) === false) {
                ...
                JLog::add(JText::_('JLIB_CLIENT_ERROR_JFTP_LISTNAMES_BAD_RESPONSE'), JLog::WARNING, 'com_sptransfer');
        ...
        if (!$this->_putCmd('NLST' . $path, array(150, 125))) {
            ...
            JLog::add(JText::sprintf('JLIB_CLIENT_ERROR_JFTP_LISTNAMES_BAD_RESPONSE_NLST', $this->_response, $path), JLog::WARNING, 'com_sptransfer');
        ...
        // Everything go okay?
        if (!$this->_verifyResponse(226)) {
            JLog::add(JText::sprintf('JLIB_CLIENT_ERROR_JFTP_LISTNAMES_BAD_RESPONSE_TRANSFER', $this->_response, $path), JLog::WARNING, 'com_sptransfer');
     ...
     * @since   12.1
     */
    public function listDetails_old($path = null, $type = 'all') {
        $dir_list = array();
        $data = null;
        $regs = null;
        ...
        if (FTP_NATIVE) {
            // Turn passive mode on
            if (@ftp_pasv($this->_conn, true) === false) {
                JLog::add(JText::_('JLIB_CLIENT_ERROR_JFTP_LISTDETAILS_PASSIVE'), JLog::WARNING, 'com_sptransfer');

                return false;
            }

            if (($contents = @ftp_rawlist($this->_conn, $path)) === false) {
                JLog::add(JText::_('JLIB_CLIENT_ERROR_JFTP_LISTDETAILS_BAD_RESPONSE'), JLog::WARNING, 'com_sptransfer');
            ...
            if (!$this->_putCmd(($recurse == true) ? 'LIST -R' : 'LIST' . $path, array(150, 125))) {
                JLog::add(JText::sprintf('JLIB_CLIENT_ERROR_JFTP_LISTDETAILS_BAD_RESPONSE_LIST', $this->_response, $path), JLog::WARNING, 'com_sptransfer');
                @ fclose($this->_dataconn);
            ...
            // Everything go okay?
            if (!$this->_verifyResponse(226)) {
                JLog::add(JText::sprintf('JLIB_CLIENT_ERROR_JFTP_LISTDETAILS_BAD_RESPONSE_TRANSFER', $this->_response, $path), JLog::WARNING, 'com_sptransfer');
        ...
        if (!$osType) {
            JLog::add(JText::_('JLIB_CLIENT_ERROR_JFTP_LISTDETAILS_UNRECOGNISED'), JLog::WARNING, 'com_sptransfer');
    ...
    public function listDetails($path = null, $type = 'all') {
        ...
        if ($osType == 'UNIX' || $osType == 'MAC') {
            foreach ($contents as $file) {
                ...
                list($tmp_array['rights'], $tmp_array['number'], $tmp_array['user'], $tmp_array['group'], $tmp_array['size'], $tmp_array['month'], $tmp_array['day'], $tmp_array['time']) = $chunks; 
                $tmp_array['type'] = $chunks[0]{0} === 'd' ? '1' : '0'; 
                array_splice($chunks, 0, 8); 
                $tmp_array['name'] = implode(" ", $chunks);
    ...
    protected function _putCmd($cmd, $expectedResponse) {
        ...
        if (!is_resource($this->_conn)) {
            JLog::add(JText::_('JLIB_CLIENT_ERROR_JFTP_PUTCMD_UNCONNECTED'), JLog::WARNING, 'com_sptransfer');
        ...
        if (!fwrite($this->_conn, $cmd . "\r\n")) {
            JLog::add(JText::sprintf('DDD', JText::sprintf('JLIB_CLIENT_ERROR_JFTP_PUTCMD_SEND', $cmd)), JLog::WARNING, 'com_sptransfer');
        }
    ...
    protected function _verifyResponse($expected) {
        ...
        if (!isset($parts[1])) {
            JLog::add(JText::sprintf('JLIB_CLIENT_ERROR_JFTP_VERIFYRESPONSE', $this->_response), JLog::WARNING, 'com_sptransfer');
    ...
    protected function _passive() {
        ...
        // Make sure we have a connection to the server
        if (!is_resource($this->_conn)) {
            JLog::add(JText::_('JLIB_CLIENT_ERROR_JFTP_PASSIVE_CONNECT_PORT'), JLog::WARNING, 'com_sptransfer');
        ...
        // Catch a timeout or bad response
        if (!isset($parts[1])) {
            JLog::add(JText::sprintf('JLIB_CLIENT_ERROR_JFTP_PASSIVE_RESPONSE', $this->_response), JLog::WARNING, 'com_sptransfer');
        ...
        if ($this->_responseCode != '227') {
            JLog::add(JText::sprintf('JLIB_CLIENT_ERROR_JFTP_PASSIVE_IP_OBTAIN', $this->_responseMsg), JLog::WARNING, 'com_sptransfer');
        ...
        if (preg_match('~\((\d+),\s*(\d+),\s*(\d+),\s*(\d+),\s*(\d+)(?:,\s*(\d+))\)~', $this->_responseMsg, $match) == 0) {
            JLog::add(JText::sprintf('JLIB_CLIENT_ERROR_JFTP_PASSIVE_IP_VALID', $this->_responseMsg), JLog::WARNING, 'com_sptransfer');
        ...
        if (!$this->_dataconn) {
            JLog::add(
                    JText::sprintf('JLIB_CLIENT_ERROR_JFTP_PASSIVE_CONNECT', $this->_pasv['ip'], $this->_pasv['port'], $errno, $err), JLog::WARNING, 'com_sptransfer'
            );
    ...
    protected function _mode($mode) {
        if ($mode == FTP_BINARY) {
            if (!$this->_putCmd("TYPE I", 200)) {
                JLog::add(JText::sprintf('JLIB_CLIENT_ERROR_JFTP_MODE_BINARY', $this->_response), JLog::WARNING, 'com_sptransfer');

                return false;
            }
        } else {
            if (!$this->_putCmd("TYPE A", 200)) {
                JLog::add(JText::sprintf('JLIB_CLIENT_ERROR_JFTP_MODE_ASCII', $this->_response), JLog::WARNING, 'com_sptransfer');
...
class SPTransferModelFiles extends JModelLegacy {
    ...
    public function getItemsRemote($folder_remote = null) {
        ...
        if (!extension_loaded('ftp')) {
            JError::raiseWarning('31', JText::_('COM_SPTRANSFER_MSG_ERROR_FTP_EXTENSION_NOT_LOADED'));
            return false;
        }
        ...
        if (!$ftp->chdir($basePath)) {
            CYENDFactory::enqueueMessage(JText::sprintf('COM_SPTRANSFER_ERROR_PATH', $basePath), 'error');
            return false;
        }
    ...
    public function transfer($folder_remote = null, $folder_local = null, $items = null) {
        ...
        if (is_null($items)) {
            ...
            foreach ($ids as $id) {
                $tmp = new JObject();
                $tmp->type = substr($id, 0, 1);
                $tmp->name = substr($id, 1);
                $items[] = $tmp;
            }
        }
        ...
        $params = JComponentHelper::getParams(CYENDFactory::getComponentName());
        $root = str_replace('//', '/', $params->get("ftp_root", '/'));
        ...
        foreach ($items as $item) {
            ...
            if ($item->type == 0) {
                ...
                if ($replace == 1 && JFile::exists($localPath . '/' . $item->name)) {
                    JLog::add(JText::sprintf('COM_SPTRANSFER_COPY_FILE_REPLACE', $localPath . '/' . $item->name), JLog::NOTICE, 'com_sptransfer');
                    continue;
                } else {
                    if (!$ftp->get($localPath . '/' . $item->name, $item->name)) {
                        jexit('<p><font color="red">' . JText::sprintf('COM_SPTRANSFER_COPY_FILE_ERROR', $localPath . '/' . $item->name) . '</font></p>');
                    } else {
                        JLog::add(JText::sprintf('COM_SPTRANSFER_COPY_FILE_SUCCESS', $localPath . '/' . $item->name), JLog::NOTICE, 'com_sptransfer');
                    }
                }
            } else {
                //folder
                if ($item->name != '..') {
                    ...
                    if (!file_exists($localPath . '/' . $item->name)) {
                        if (!JFolder::create($localPath . '/' . $item->name)) {
                            jexit('<p><font color="red">' . JText::sprintf('COM_SPTRANSFER_CREATE_FOLDER_ERROR', $localPath . '/' . $item->name) . '</font></p>');
                        } else {
                            JLog::add(JText::sprintf('COM_SPTRANSFER_CREATE_FOLDER_SUCCESS', $localPath . '/' . $item->name), JLog::NOTICE, 'com_sptransfer');
                        }
                    }
    ...
    protected function ftp_get_folder($items, $ftp) {
