...
$id = JRequest::getVar('cid', array(0) );
  if (!is_array( $id )) {
  $id = JRequest::getVar('cid', array(0), "get" );
  if (is_array( $id )) {
    $id = array(0);
  } else {
    $id = array($id);
...
require_once(JPATH_COMPONENT_ADMINISTRATOR . '/jfuploader.class.php');
require_once(JPATH_COMPONENT_ADMINISTRATOR . '/admin.jfuploader.html.php');

require_once(JPATH_COMPONENT_ADMINISTRATOR . '/toolbar.jfuploader.php');
...
$database = JFactory::getDBO();
 ...
 $highlightConfig = false;
  ...
  //  echo "a:" . $view . " ID " . $id[0];
  switch ($view) {
    case "upload": showUpload(); $highlightUpload=true; break;
    case "config": showConfig(); $highlightConfig=true; break;
    case "plugins": showPlugins(); $highlightPlugin=true; break;
    case "edit": showConfigUser($id[0]); $highlightConfig=true; break;
    case "edituser": showConfigUser($id[0], true); $highlightConfig=true; break;
    case "deleteConfig": deleteConfig($id); $highlightConfig=true; break;
    case "newConfig": newConfig();  $highlightConfig=true; break;   
    case "saveConfig": saveConfig(false); $highlightConfig=true; break;
    case "saveConfigClose": saveConfig(true); $highlightConfig=true; break;
    case "saveMainConfig": saveMainConfig(); $highlightConfig=true; break;
    case "copyConfig": copyConfig($id); $highlightConfig=true; break;
    case "addUser": addUser();  $highlightUser=true; break;
    case "deleteUser": deleteUser($id);  $highlightUser=true; break;
    case "cancel": cancel(); break; 
    case "register": register(); $highlightHelp=true; break; 
    case "dellic": deleteLicense(); $highlightHelp=true; break; 
    case "help": showHelpRegister(); $highlightHelp=true; break;
    case "user": showUser($id);  $highlightUser=true; break;  
    case "createhtaccess" : createHtaccess(); $highlightHelp=true; break;
    case "deletehtaccess" : deleteHtaccess(); $highlightHelp=true; break;
    case "deletelog" : deleteLog(); $highlightHelp=true; break;
    case "changeProfile" : changeProfile();  $highlightConfig=true; break;
    case "changeMaster" : changeMaster();  $highlightConfig=true; break;
    case "testFolder" : testFolder(); break;
    case "chmod777" : chmod_tfu(0777); $highlightHelp=true; break;
    case "chmod666" : chmod_tfu(0666); $highlightHelp=true; break;
    case "chmod755" : chmod_tfu(0755); $highlightHelp=true; break;
    case "chmod644" : chmod_tfu(0644); $highlightHelp=true; break;
    case "movetfudir" : jfu_move_tfu_dir();  $highlightConfig=true; break;
    case "addGroup": addUserGroup('site', 'jgroup', 'profilegroup');  $highlightUser=true; break;
    case "addBackGroup": addUserGroup('admin', 'jgroupback', 'allprofile');  $highlightUser=true; break;
    
    default: showUpload();  $highlightUpload=true; break;
  }
  
  if (checkAccess($database, 'core.manage' )) {
  JSubMenuHelper::addEntry(JText::_('JFU_M_1'), 'index.php?option=com_jfuploader&view=upload', $highlightUpload);
}
if (checkAccess($database, 'core.admin' )) {
  JSubMenuHelper::addEntry(JText::_('JFU_M_2'), 'index.php?option=com_jfuploader&view=config',$highlightConfig);
}
if (checkAccess($database, 'core.edit.state' ) && checkAccess($database, 'core.admin' )) {
  JSubMenuHelper::addEntry(JText::_('JFU_M_3'), 'index.php?option=com_jfuploader&view=user',$highlightUser);
}
if (checkAccess($database,'core.admin')) {
  JSubMenuHelper::addEntry(JText::_('JFU_M_5'), 'index.php?option=com_jfuploader&view=plugins', $highlightPlugin);
}
if (checkAccess($database,'core.admin') || checkAccess($database,'core.manage')) {
  JSubMenuHelper::addEntry(JText::_('JFU_M_4'), 'index.php?option=com_jfuploader&view=help',$highlightHelp);
}
...
function newConfig() {
    ...
    if (checkAccess($database, 'core.create' )) {
      ...
      $row->last_modified_date = date("Y-m-d");
	    $row = JFUHelper::convertAllIfBoolean($row);
...
function deleteConfig($cid) {
   ...
   $database->query();
   $mainframe->redirect( "index.php?option=com_jfuploader&view=config" );
    } else {
     HTML_joomla_flash_uploader::errorRights();
  }
} 
...
function saveMainConfig() {
  ...
  $kt = JFUHelper::convertIfBoolean('is_boolean', JRequest::getVar('keep_tables', 'true' ));
  $uj = JFUHelper::convertIfBoolean('is_boolean', JRequest::getVar('use_js_include', 'true' ));
  ...
  $up = JFUHelper::convertIfBoolean('is_boolean', JRequest::getVar('enable_upload_debug', 'false' ));
  $sp = JRequest::getVar('sa_profil', '1' );
  $ap = JRequest::getVar('a_profil', '1' );
  $mp = JRequest::getVar('m_profil', '1' );
  $ed = JFUHelper::convertIfBoolean('is_boolean', JRequest::getVar('enhanced_debug', 'false' ));
  $im = JFUHelper::convertIfBoolean('is_boolean', JRequest::getVar('check_image_magic', 'true' ));
  $id = JRequest::getVar('idn_url', '' );
  $ui = JRequest::getVar('use_index_for_files', 'true' );
  ...
  $database->query();
  cleanMessageQueue();
  $mainframe->redirect( "index.php?option=com_jfuploader&view=config", JText::_('MES_SAVED'));
   } else {
     HTML_joomla_flash_uploader::errorRights();
  }
}
...
$mainframe = JFactory::getApplication(); 
    ...
    if (checkAccess($database, 'core.create' )) {
  	...
  	if (count($cid) == 0) {
  	      cleanMessageQueue();
	  	  $mainframe->redirect("index.php?option=com_jfuploader&view=config", JText::_('MES_COPY_NONE'));
	  	  return;
  	}
  	if (count($cid) > 1) {
  	  cleanMessageQueue();
  	  $mainframe->redirect("index.php?option=com_jfuploader&view=config", JText::_('MES_COPY_ONE'));
  	  return;
  	}
  	$row = new joomla_flash_uploader($database);
    $row->load($cid[0]);
    ...
    $row->store();
    cleanMessageQueue();
    $mainframe->redirect("index.php?option=com_jfuploader&view=config", JText::_('MES_COPY_OK'));
   } else {
     HTML_joomla_flash_uploader::errorRights();
  }
}
...
function showConfig() {
  ...
  $jfu_config['idn_url']= JFUHelper::getVariable($database, 'idn_url');
  $jfu_config['use_index_for_files']= JFUHelper::getVariable($database, 'use_index_for_files', false);
	...
	$rows = $database->loadObjectList();
	  ...
	  if (count($profiles) != 0) {
	    ...
	    $data['profilesgroup'] = JHTML::_('select.genericlist', $profiles, 'profilegroup', 'size="1"', 'id', 'config_name', $last_profile) . '<div style="clear:both"></div><p>'.JText::_('U_AVAILABLE_LIST').'.</p>';
         ...
         $data['profiles'] = $data['profilesgroup'] = '<div class="message">'.JText::_('U_NO_PROFILE').'.</div>';
         $data['showAdd'] = false;
       }
...
$row = JFUHelper::convertAllIfTrueFalse($row);
$row->last_modified_date=date("Y-m-d");
$row->store();
...
if ($userchanged == 'yes') {
  ...
  $database->setQuery( "DELETE FROM #__joomla_flash_uploader_user WHERE location='site' and user is not null and profile = ($row->id)" );
  $database->query(); 
...
// remove all user mappings for groups
$database->setQuery( "DELETE FROM #__joomla_flash_uploader_user WHERE jgroup is not null and profile = ($row->id)" );
$database->query(); 
...
if ($closeit) {
  ...
  $mainframe->redirect("index.php?option=com_jfuploader&view=edit&cid=" . $row->id, JText::_('MES_SAVED'));
}
}
...
if (!isset($_POST['user']) || !isset($_POST['profile'])) {
  $mainframe->redirect("index.php?option=com_jfuploader&view=user", JText::_('MES_MAP_NOSEL'));
}
...
if ($error_num > 0) {
  $mainframe->redirect("index.php?option=com_jfuploader&view=user", $error_num . JText::_('MES_EXISTS'));
} else {
  $mainframe->redirect("index.php?option=com_jfuploader&view=user", JText::_('MES_MAP_SAVED'));
}
}
...
$mainframe = JFactory::getApplication(); 
   ...
   $database->query(); 
   cleanMessageQueue();
   $mainframe->redirect( "index.php?option=com_jfuploader&view=user", JText::_('MES_MAP_REM'));
} 
...
$mainframe = JFactory::getApplication(); 
  ...
  if (file_exists($filename)) {
     $mainframe->redirect( "index.php?option=com_jfuploader&view=upload", JText::_('MES_HTACCESS_CREATED') ); 
  }  else {
     $mainframe->redirect( "index.php?option=com_jfuploader&view=upload", JText::_('MES_HTACCESS_NOT_CREATED') );
  }
}
...
$mainframe = JFactory::getApplication(); 
  ...
  @unlink($file);
  cleanMessageQueue();
  $mainframe->redirect( "index.php?option=com_jfuploader&view=upload", JText::_('MES_HTACCESS_DELETED') );
}
...
$mainframe = JFactory::getApplication(); 
  ...
  @unlink($file);
  cleanMessageQueue();
  $mainframe->redirect( "index.php?option=com_jfuploader&view=help", JText::_('MES_LOG_DELETED') );
}
...
function deleteLicense() {
 ...
 $mainframe = JFactory::getApplication(); 
  ...
  @unlink($file);
  cleanMessageQueue();
  $mainframe->redirect( "index.php?option=com_jfuploader&view=help", JText::_('MES_LICENSE_DELETED') );
}
...
function chmod_tfu($mode) {
  ...
  if (checkAccess($database,'core.admin')) {
    ...
    cleanMessageQueue();
    $mainframe->redirect( "index.php?option=com_jfuploader&view=help", JText::_('H_L_CHMOD_MES') );
  } else {
    HTML_joomla_flash_uploader::errorRights();
  }  
}
...
$mainframe = JFactory::getApplication(); 
 ...
 $s =  trim(JRequest::getVar( 's','' ));
  ...
  cleanMessageQueue();
  $mainframe->redirect( "index.php?option=com_jfuploader&view=help", $text );
}
...
$mainframe = JFactory::getApplication(); 
  ...
  if (rename($filename, $otherfilename )) {
     clearstatcache();
     $mainframe->redirect( 'index.php?option=com_jfuploader&view=config', JText::_('MES_TFU_MOVED') ); 
  }  else {
     $mainframe->redirect( 'index.php?option=com_jfuploader&view=config', JText::_('MES_TFU_NOT_MOVED') );
  }  
}
