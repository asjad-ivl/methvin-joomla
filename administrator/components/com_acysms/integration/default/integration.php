...
class ACYSMSIntegration_default_integration{
	...
	function getInformationsByPhoneNumber($phoneNumber){
		...
		if(!empty($integrationPhoneField) && !empty($phoneNumberToSearch)){
			$query = 'SELECT *, '.$this->tableAlias.'.'.$this->primaryField.' as receiver_id, '.$columnName.' as receiver_name, '.$integrationPhoneField.' as receiver_phone_number
					FROM '.$this->tableName.' AS '.$this->tableAlias.'
					LEFT JOIN #__users AS joomUsers
					ON joomUsers.id = '.$this->tableAlias.'.'.$this->joomidField.'
					WHERE `'.$integrationPhoneField.'` = '.$db->Quote($phoneNumberToSearch).' OR REPLACE(`'.$integrationPhoneField.'`, ".", "") LIKE '.$db->Quote('%'.$phoneNumberToSearch).' OR REPLACE(`'.$integrationPhoneField.'`, "-", "") LIKE '.$db->Quote('%'.$phoneNumberToSearch);
			$db->setQuery($query);
