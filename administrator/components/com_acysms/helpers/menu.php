...
class ACYSMSmenuHelper{
	function display($selected = ''){
		...
		if(ACYSMS::isAllowed($config->get('acl_answers_manage', 'all'))){
			...
			$submenu['answer'][] = array(JText::_('SMS_CONVERSATION'), 'index.php?option=com_acysms&ctrl=receiver&task=conversation', 'smsicon-conversation');
		...
		$acyMailingLink = '';
		if(ACYSMS::isAllowed($config->get('acl_configuration_manage', 'all'))) $acyMailingLink = '<a class="sendother" href="index.php?option=com_acysms&ctrl=update&task=acymailing">'.JText::_('SMS_EMAILS').'&nbsp;&nbsp;<i class="smsicon-send"></i></a>';

		$menu .= '<div class="acysmsmenu_slide"><span>'.$acyMailingLink.'<i class="smsicon-open-close" onclick="acysmsToggleClass(\'acysmsallcontent\',\'iconsonly\');"></i></span></div>';
		...
		$menu .= '<div id="myacysmsarea" class="myacysmsarea">'; //DO NOT CHANGE THIS ID! we use it for ajax things...
