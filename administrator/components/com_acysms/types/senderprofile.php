...
class ACYSMSsenderprofileType{
	...
	function display($map, $values){
		...
		if(!$app->isAdmin() && $allowCustomerManagement){
			...
			if(empty($customer)) die('Only customers can access this page. Please contact the website administrator and ask him to assign your Joomla user to a customer.');
		...
		return JHTML::_('select.genericlist', $this->values, $map, 'class="inputbox" '.$this->js.' onchange="'.$onChangeFunction.'" style="width:200px;" '.$multiple, 'senderprofile_id', 'senderprofile_name', $values);
	}
}
