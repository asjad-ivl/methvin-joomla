	...
	<table class="acysms_table">
		...
		for($i = 0, $a = count($this->rows); $i < $a; $i++){
			...
			<tr class="<?php echo "row$k"; ?>">
				...
				<td>
					<?php
					if(!empty($row->answer_receiver_id)){
						echo '<a class="modal" href="index.php?option=com_acysms&tmpl=component&ctrl='.$receiverCtrl.'&task=conversation&receiverid='.$row->answer_receiver_id.'" rel="{handler: \'iframe\', size: {x: 500, y: 600}}"><i class="smsicon-conversation"></i></a>';
					}
					?>
				</td>
			</tr>
