...
class statsViewstats extends acysmsView{
	...
	function detaillisting(){
		...
		$pagination = new JPagination($pageInfo->elements->total, $pageInfo->limit->start, $pageInfo->limit->value);
			...
			$acyToolbar = ACYSMS::get('helper.toolbar');
			$acyToolbar->setTitle(JText::_('SMS_STATS_DETAILED').': '.$messageClass->get($message_id)->message_subject, $this->icon, $this->ctrl);

			if(JRequest::getString('tmpl', '') != 'component') $acyToolbar->link(ACYSMS::completeLink('stats'), JText::_('SMS_STATS'), 'stats');
			$acyToolbar->link(ACYSMS::completeLink(($app->isAdmin() ? 'stats' : 'frontstats').'&task=exportStatsDetails&message_id='.$message_id), JText::_('SMS_EXPORT'), 'export');
			if(JRequest::getString('tmpl') == 'component') $acyToolbar->link(ACYSMS::completeLink(($app->isAdmin() ? 'stats' : 'frontstats').'&task=diagram&message_id='.JRequest::getInt('filter_message'),true), JText::_('SMS_CANCEL'), 'cancel');
