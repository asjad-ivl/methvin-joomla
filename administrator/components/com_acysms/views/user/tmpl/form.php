			...
			<table class="acysms_table" cellspacing="1" align="center">
				...
				<tr>
					...
					<?php if($this->app->isAdmin()){ ?>
						<th class="title">
							<?php echo JText::_('SMS_SUBSCRIPTION_DATE'); ?>
						</th>
						<th class="title">
							<?php echo JText::_('SMS_UNSUBSCRIPTION_DATE'); ?>
						</th>
					<?php } ?>
				...
				foreach($this->subscription as $row){
					...
					<tr class="<?php echo "row$k "; ?>">
						...
						<?php if($this->app->isAdmin()){ ?>
							<td>
								<?php echo empty($row->groupuser_subdate) ? '' : ACYSMS::getDate($row->groupuser_subdate); ?>
							</td>
							<td>
								<?php echo empty($row->groupuser_unsubdate) ? '' : ACYSMS::getDate($row->groupuser_unsubdate); ?>
							</td>
						<?php } ?>
						<td align="center">
